...
- ✅ ~~No server-side price floor~~ — corrected, this line was stale: §1.3 above already documents this as fully fixed (`priceOverrideReason` required when an admin bills below catalogue)
...
- ✅ **Fixed.** Cancellation now requires a `reason` (`CancelInvoiceSchema`, min 3 chars) and writes a semantic before/after audit diff (`{status}` → `{status: "cancelled", cancelReason}`) via the same `RequestContext.setAuditDiff` mechanism §1.4 introduced — previously the generic `AuditInterceptor` row only recorded that a POST happened, not why. New `Invoice.cancelReason`/`cancelledAt` columns. Also found and fixed while wiring this up: **the cancel UI didn't exist anywhere in the frontend** despite this doc's own §6 table claiming it did — the backend endpoint was real but unreachable from the app. Added a "Cancelar Fatura" button on the invoice detail page with the app's standard two-step inline confirmation (§4.6) and a required reason textarea, plus a cancelled-invoice banner. TDD: `billing.service.spec.ts` (+2 tests).
- ✅ **Fixed.** Payment-to-staff attribution — `Payment.recordedById` (FK to `Staff`), set from the authenticated caller in `POST /invoices/:id/payments`, shown in the invoice detail page's payment history ("registado por …"). Previously `Payment` had no field for who recorded it at all (§2.3/§7 below).
...
**Status: ✅ Fixed.** One shared `apps/web/components/ui/field.tsx` — implicit `<label>` wrapper (so `getByLabel()` is reliable everywhere, not just the one edit page it had been fixed on), superset prop signature (`label`/`required`/`error`/`hint`). All 10 local copies deleted and re-pointed at it (`appointments/new`, `patients/new`, `patients/page`, `patients/[id]/edit`, `patients/[id]/ClinicalRecordsSection`, `health-plans`, `staff/_AvailabilityCalendar`, `visits`, `exams`, plus `components/staff/StaffForm`'s `FieldRow`). `components/settings/shared.tsx` now re-exports `Field` from the new location so `settings/page.tsx`'s existing import keeps working. One deliberate visual change: `patients/[id]/edit` had a divergent uppercase-11px label style — it now matches the app-standard 12px semibold like every other form. Typecheck + lint clean on both apps.
```
...
**Status: ✅ Fixed.** New `apps/web/lib/use-debounced-value.ts` (`useDebouncedValue(value, delayMs=300)` — raw `setTimeout`, no new dependency). Wired into `patients/page.tsx`: the input stays controlled by its own state (typing is instant), the debounced value drives the React Query key + fetcher, and the page-1 reset now fires when the debounced term settles rather than on every keystroke. Checked every other search box in the app — the only other free-text ones (`whatsapp`, `exams`) are mockups that filter a hardcoded client-side array with no network call, so nothing to debounce there; the hook is in place for whenever those get real backends.

### 4.3 — The global Zod validation pipe is a no-op
**Status: ✅ Fixed (removed).** `app.useGlobalPipes(new ZodValidationPipe())` deleted from `main.ts` (with a comment there explaining validation is per-route). `ZodValidationPipe`'s constructor `schema` is now required, not optional — a future no-schema instantiation won't compile — and the `if (!this.schema) return value` no-op branch is gone. All 49 per-route `new ZodValidationPipe(Schema)` uses across all 13 controllers are unaffected. API typecheck + 355 tests green.
...
**Status: ✅ Fixed.** Every raw `console.*` in `apps/api/src` is now a `Logger` (the existing `private readonly logger = new Logger(X.name)` pattern): `appointments.service.ts` (×2, auto-invoice + health-plan-usage failure paths — now `logger.error` with the stack), `notifications.processor.ts` (WhatsApp send failure), `appointments/reminders.processor.ts` (the Phase-1 stub log), `http-exception.filter.ts` (unhandled-exception path), and `main.ts` bootstrap (a `Logger("Bootstrap")` — `console.warn`-to-force-visibility replaced with proper `log`/`warn`/`error`). API lint clean.

### 4.5 — Test coverage: 7 of 14 API modules have zero unit tests
**Status: ✅ Closed.** Every API module now has a service-level spec. The last four added:
- `services.service.spec.ts` (10) — code-uniqueness on create, the same-code-no-dup-check carve-out on update, NotFound guards, soft-delete
- `companies.service.spec.ts` (7) — taxId uniqueness, `activeOnly` default, the lightweight `exists()` check on deactivate
- `parametrizacao.service.spec.ts` (9) — active-only vs. include-inactive read paths, the `findById` NotFound guard on update/softDelete
- `public.service.spec.ts` (7) — delegation to the 4 wrapped services, the `getStaff` public-field projection, and `createBooking`'s find-or-create → book → `bookingReference` (first 8 id chars, uppercased) orchestration
```
Every API module has a *.service.spec.ts. (bff has bff.service.spec.ts.)
```
...
**Status: ✅ Fixed — at the schema layer, so client and server agree.** Three reusable validators now live in `@cap/types` `common.ts` and are used by both `CreatePatientSchema`/`UpdatePatientSchema` and `PublicBookingSchema`:
- `dateOfBirthSchema` — `YYYY-MM-DD`, a real calendar date, **not in the future**, not before 1900. The three patient forms (`patients/new`, the `patients/page` modal, `patients/[id]/edit`) also got `max={today}` on the `<input type="date">` for immediate client-side pushback.
- `nifSchema` — exactly 9 digits (was `min(6).max(20)`, "accepts anything"). Forms show a `9 dígitos` hint + `inputMode="numeric" maxLength={9}`.
- `caboVerdePhoneSchema` — backed by a shared `normalizeCaboVerdePhone()` helper that `PatientsService.normalizePhone` now also calls, so the "7 digits, with or without +238" rule has exactly one definition instead of the form saying "E.164" while the server enforced something stricter. Forms show a `+238 + 7 dígitos` hint.
- Bonus: `patients/[id]/edit` (plain `useState`, no `zodResolver`) now renders the API's array-shaped Zod error messages as readable text instead of `[object Object]`.
TDD: covered by the existing `patients.service.spec.ts` phone-normalisation suite (39 tests green — behaviour identical after the refactor). typecheck + lint clean on all three packages.
...
- [x] Testing — 7/14 API modules covered originally; **now every API module has a service-level spec** (last four added: `services`, `companies`, `parametrizacao`, `public`), plus `common/guards`, `common/interceptors`, `common/context`, `common/services` and repository-level specs for patients/billing.
- [x] Code Quality §4.1–4.4 — **fixed**: one shared `components/ui/field.tsx` (implicit label wrapper) replacing 10 copies; `useDebouncedValue` hook wired into patient search; the no-op global `ZodValidationPipe` removed from `main.ts` (schema now required); all `apps/api` `console.*` moved to NestJS `Logger`.
- [x] Code Quality §4.5 — **closed**: `services`, `companies`, `parametrizacao`, `public` service specs added (33 tests). Every API module now has a service-level spec.
- [x] UX §5.1 — **fixed**: shared `dateOfBirthSchema` (no future dates) / `nifSchema` (9 digits) / `caboVerdePhoneSchema` in `@cap/types`, used by the patient forms + public-booking schema; phone rule de-duplicated against `PatientsService`'s own normaliser; `max={today}` + format hints on the 3 patient forms. §5.2/5.3/5.4/5.5 were already fine/fixed.

**Section 1 (Critical Findings) — all 7 items fully addressed**, including every "deliberately left out" follow-up except MFA retroactive enforcement (blocked on a real Keycloak deployment existing — the fix for new accounts is done). **Section 2 (Privacy & Compliance) — the 4 fixable-in-code items are also all fully addressed** (DOB encryption, audit-log immutability, right-to-erasure PII clearing, consent-bypass); the remaining rows (MFA enforcement, exam-link time-limiting, patient self-service login) are blocked on infrastructure/features that don't exist yet, not code defects to fix in place. **Section 3 (Phase-by-Phase Checklist) — the scoped bug-fix pass is complete**: every concrete, fixable-in-place gap across M1/M2/M6/M8 is closed, plus two documentation-drift corrections and a permanent roadmap cut (DICOM viewer). Deliberately **not** touched, per explicit scope agreed before starting: the five modules that are entirely UI mockups with no backend at all (M3 WhatsApp, M5 Exams, M7 Clinical Records, M9 Home Visits, M10 Analytics) and all of Phase 4 — those are new-feature builds, not bug fixes, and each needs its own scoping conversation. Also explicitly deferred within the fixed modules: M1's reminder-channel hardcoding (needs new SMS-sending infrastructure that doesn't exist anywhere in the codebase). 386 API tests passing, typecheck clean on both apps, every fix either live-verified against the running dev stack or, where a live check wasn't practical (e.g. a job that runs against real wall-clock time), covered by TDD alone.

**2026-09-11 follow-up session** — targeted M6 correctness pass: payment-to-staff attribution (`Payment.recordedById`) and invoice-cancel reason + audit diff, plus a cancel-invoice frontend UI that turned out not to exist at all despite §6 below claiming it did. Also corrected three stale doc references found along the way: §1.3's price floor was already fully fixed (this file's own bug-fix-pass summary above still called it open), the stale-receipt-on-repayment bug in §2.4 was already fixed in a prior commit (`recordPaymentAtomic` nulls `pdfR2Key` on every payment), and `TODO.md`'s "New invoice form" line was stale (`billing/new/page.tsx` is real). 389/389 tests, typecheck + lint clean.

**Section 4 (Code Quality & Best Practices) — all of §4.1–4.5 done**: shared `Field` component, `useDebouncedValue` search hook, no-op global Zod pipe removed, all `apps/api` `console.*` calls standardized to NestJS `Logger`, and service specs for the last four untested modules (`services`, `companies`, `parametrizacao`, `public`) — every API module now has one. No behavior change — 386 API tests green, both apps typecheck + lint clean.

With Section 5 already closed and Section 6 being a redesign exercise rather than a fix list, **nothing in this review remains as an actionable code change.**

**Section 5 (UX/UI Findings) — done**: §5.1 (the only open item — §5.2/5.3/5.5 were verified-fine, §5.4 already fixed) is closed. DOB/NIF/phone validation now lives in shared `@cap/types` schema fragments used by both patient forms and the public-booking schema, with the phone rule de-duplicated against the server's own normaliser; the 3 patient forms also got `max={today}` on the date input and format hints.

**§4.5 — closed**: `services`, `companies`, `parametrizacao`, and `public` all got service specs (33 tests). Every API module now has one. Section 6 is redesign suggestions — explicitly *not* an implementation task — so nothing in the review remains actionable as a fix.
