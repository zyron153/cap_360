...
**Status: 🟡 Still open — deferred by explicit scope decision.** The 4.1–4.4 pass covered the concrete code-quality fixes; adding fresh spec suites for the 4 modules still at zero coverage is its own effort, left for a follow-up. The gap is much smaller than when this was written — `bff`, `documents`, `health-plans`, `notifications`, and `staff` all have specs now.
```
Has specs:    appointments, auth, bff, billing, clinical-records, documents, efatura (×2),
              financeiro, health-plans, notifications, patients, settings, staff
No specs at all: companies, parametrizacao, public, services
```
...
- [x] Code Quality §4.1–4.4 — **fixed**: one shared `components/ui/field.tsx` (implicit label wrapper) replacing 10 copies; `useDebouncedValue` hook wired into patient search; the no-op global `ZodValidationPipe` removed from `main.ts` (schema now required); all `apps/api` `console.*` moved to NestJS `Logger`. §4.5 (spec suites for `companies`/`parametrizacao`/`public`/`services`) deferred.
