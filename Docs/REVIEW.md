...
**Section 4 (Code Quality & Best Practices) — §4.1–4.4 done** (per an explicit scope decision to leave §4.5's new spec suites for a follow-up): shared `Field` component, `useDebouncedValue` search hook, no-op global Zod pipe removed, and all `apps/api` `console.*` calls standardized to NestJS `Logger`. No behavior change — 355 API tests still green, both apps typecheck + lint clean. Next: Section 5 (UX/UI Findings) and Section 6 (Redesign Suggestions), plus §4.5, if/when asked to continue.
