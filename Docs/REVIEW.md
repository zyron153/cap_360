...
**Status: 🟡 Mostly closed — 2 of the last 4 modules now covered.** `services.service.spec.ts` (10 tests — code-uniqueness on create, the same-code-no-dup-check carve-out on update, NotFound guards, soft-delete) and `companies.service.spec.ts` (7 tests — taxId uniqueness, `activeOnly` default, the lightweight `exists()` check on deactivate). `parametrizacao` and `public` remain uncovered — both are thin delegation layers, left as a small follow-up.
```
Has specs:    appointments, auth, bff, billing, clinical-records, companies, documents,
              efatura (×2), financeiro, health-plans, notifications, patients, services,
              settings, staff
No specs at all: parametrizacao, public
```
...
- [x] Testing — 7/14 API modules covered originally; now also covers `common/guards`, `common/interceptors`, `common/context`, `common/services`, repository-level specs for patients/billing, and new service-level specs for staff, notifications, services, and companies (all previously untested). Only `parametrizacao` + `public` remain uncovered.
...
- [~] Code Quality §4.5 — `services` + `companies` service specs added (17 tests); `parametrizacao` + `public` still uncovered (thin delegation layers).
...
**§4.5 — 2 of the last 4 modules covered** (`services`, `companies` — 17 tests). `parametrizacao` and `public` (thin delegation layers) remain the only two untested modules. Section 6 is redesign suggestions — explicitly *not* an implementation task.
